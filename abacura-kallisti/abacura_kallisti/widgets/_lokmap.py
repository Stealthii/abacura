...
TERRAINS: list[TerrainStyle] = [
    ...
    TerrainStyle(
        "Mountains",
        "◭",
        ColorTriplet(59, 38, 4),
        ColorTriplet(121, 110, 91),
    ),  # ColorTriplet(200, 190, 190) ColorTriplet(59, 38, 4)),  # ◣▴^  △ΔΛΛ▲⏶▴◭
...
class LOKMap(Container):
    ...
    def update_map(self):
        ...
        match map_type:
            ...
            case "3x3":
                self.generate_3x3_map(self.start_room, width, height)
            case "1x1":
                self.generate_1x1_map(self.start_room, width, height)
            case "wilderness":
                self.generate_wilderness_map(self.start_room, width, height)
    ...
    def make_5x3_top_row(self, room: Optional[Room]) -> list[Segment]:
        ...
        return [
            Segment("  ", Style(bgcolor=style.bgcolor)),
    ...
    def make_5x3_mid_row(self, room: Optional[Room]) -> list[Segment]:
        ...
        style = self.terrain_style_for_terrain(room.terrain_name).style
        return [
            Segment("-[" if "west" in room.exits else " [", Style(bgcolor=style.bgcolor)),
