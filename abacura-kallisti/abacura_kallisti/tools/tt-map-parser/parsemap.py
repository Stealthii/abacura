...
CREATE TABLE rooms(vnum,name,terrain_name,area_name,regen_hp,regen_mp,regen_sp,set_recall,peaceful,deathtrap,
    silent,wild_magic,bank,narrow,no_magic,no_recall, last_visited, last_harvested, primary key (vnum))
...
class RoomRecord:
    insert_statement = (
        "INSERT OR IGNORE INTO rooms(vnum,name,terrain_name,area_name,regen_hp,regen_mp,regen_sp,set_recall,peaceful,deathtrap,"
        "silent,wild_magic,bank,narrow,no_magic,no_recall,last_visited,last_harvested)"
        " VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
    )
...
class ExitRecord:
    insert_statement = (
        "INSERT OR IGNORE INTO exits(from_vnum,direction,to_vnum,door,closes,locks,key_name,weight,max_level,"
        "min_level,deathtrap, commands)"
        " VALUES(?,?,?,?,?,?,?,?,?,?,?,?)"
    )
...
def main(infile: str, outfile: str) -> None:
    ...
    cur.execute(
        ...
        "exits(from_vnum,direction,to_vnum,door,closes,locks,key_name,weight,max_level,min_level,deathtrap,commands, "
        "PRIMARY KEY (from_vnum, direction))",
    )
    cur.execute(
        "CREATE TABLE IF NOT EXISTS "
        "rooms(vnum,name,terrain_name,area_name,regen_hp,regen_mp,regen_sp,set_recall,peaceful,deathtrap,silent,"
        "wild_magic,bank,narrow,no_magic,no_recall, last_visited, last_harvested, "
        "primary key (vnum))",
    )
        ...
        for line in fh:
            if line:
                ...
                if line.startswith("R"):
                    ...
                    r = RoomRecord()
                    r.vnum = vals[0]
                    # We stop if we get a vnum over 61000, currently excluding wilderness,
                    # and abacura's automapper will map anywhere with vnums we encounter there
                    if int(r.vnum) > 61000:
                        print("Vnum over 61k:", r.vnum)
                        con.commit()
                        con.close()
                        # close file handler too
                        print(f"Total of {count['rooms']} rooms and {count['exits']} exits processed.")
                        ...
                        print(f"Does {count['rooms'] + count['exits']} == {count['total']}?")
                        exit()
                    r.name = vals[2]
                    if len(r.name) == 0:
                        last_rnum = None
...
we want to port this into commands, and anything that's enter/turn/say,
the direction should be the second word, such as tent, clan, sundial.
